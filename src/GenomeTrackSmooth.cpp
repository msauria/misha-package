...
void Linear_ramp_smoother::set_next_sample(double sample)
{
	if (m_num_read_samples > m_num_samples_aside) {
		...
		if ((!m_smooth_nans && __builtin_isnan(m_vals[m_peak_idx])) || !weight || weight < m_weight_thr)
	...
	if (__builtin_isnan(m_vals[m_left_idx]))
		m_left_nans--;
	else
		m_left_vals_sum -= m_vals[m_left_idx];

	// shift the peak element left
	if (__builtin_isnan(m_vals[m_peak_idx])) {
		m_left_nans++;
		m_right_nans--;
	}
	else {
...
{
	if (m_num_read_samples > m_num_samples_aside) {
		float mean = (!m_smooth_nans && __builtin_isnan(m_vals[m_peak_idx])) || !m_weights_sum || m_weights_sum < m_weight_thr ? numeric_limits<double>::quiet_NaN() : m_vals_sum / m_weights_sum;
