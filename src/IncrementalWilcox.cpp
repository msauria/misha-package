...
void IncrementalWilcox::update(double old_v1, double new_v1, double old_v2, double new_v2)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		if ((__builtin_isnan(old_v) && __builtin_isnan(new_v)) || (!__builtin_isnan(old_v) && !__builtin_isnan(new_v) && old_v == new_v))
			continue;
